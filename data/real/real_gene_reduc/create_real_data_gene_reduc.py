...
##### REMOVE GENES #####
genes_to_remove = ['NR4A2', 'MYC', 'FGFR1', 'TACC1']

print(f"shapes before dropping genes: healthy {df_clean_healthy.shape}, pure disease {df_pure_combined.shape}, disease {df_combined_disease.shape}")
df_clean_healthy = df_clean_healthy.drop(index=genes_to_remove)
df_combined_disease = df_combined_disease.drop(index=genes_to_remove)
df_pure_combined = df_pure_combined.drop(index=genes_to_remove)

print(f"shapes after dropping genes: healthy {df_clean_healthy.shape}, pure disease {df_pure_combined.shape}, disease {df_combined_disease.shape}")
